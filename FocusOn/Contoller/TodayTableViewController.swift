...
class TodayTableViewController: UITableViewController, UITextViewDelegate {
  ...
  private func displayAlertAction() {
    let alert = UIAlertController(title: "Your goal has not been achieved yet!", message: "Would you like to re-list it as today's goal?", preferredStyle: UIAlertController.Style.alert)
  ...
  func textViewDidChange(_ textView: UITextView) {
    // begin and end updates of the tableview to make sure cells are expanding simultaneously with the caption text inside the textView
    tableView.beginUpdates()
    tableView.endUpdates()
    if WeDoNotHaveA(goal: goal) {
  ...
  func WeDoNotHaveA(goal: ToDo?) -> Bool {
